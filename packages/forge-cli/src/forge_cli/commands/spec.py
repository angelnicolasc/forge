...
def spec_init(
    ...
    output: Path | None = typer.Option(
        ...
        "--output",
        "-o",
        help="Output file (default: <name>.yaml)",
    ),
) -> None:
...
def spec_verify(
    ...
    output_text: str = typer.Option(
        "",
        "--output",
        help="Simulate run output for CONTAINS/REGEX checks",
    ),
    status: str = typer.Option(
        "completed",
        "--status",
        help="Simulate run status (completed/failed)",
    ),
) -> None:
    ...
    try:
        ...
        err_console.print(
            f"[red]Invalid status:[/red] {status!r}. Use: completed, failed, cancelled"
        )
        raise typer.Exit(code=1) from None

    run_result = RunResult(
        task_id="cli", run_id=run_id, status=run_status, output=output_text or None
    )
    vr = asyncio.run(forge_spec.SpecVerifier().verify(spec, run_result))

    console.print(f"\n[bold]Spec:[/bold] {spec.name} v{spec.version}")
    console.print(
        f"[bold]Compliance:[/bold] {vr.compliance_rate:.0%}  [bold]Passed:[/bold] {vr.passed}"
    )
...
def spec_attest(
    ...
    compliance_rate: float = typer.Option(
        ...,
        "--compliance-rate",
        help="Compliance rate [0.0–1.0]",
    ),
    ...
    output: Path | None = typer.Option(
        ...
        "--output",
        "-o",
        help="Write attestation JSON to file",
    ),
) -> None:
